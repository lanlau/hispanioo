    ...
    //export let categories=null
      ...
      <div class=" {$$props.class}">
        <div class=" bg-white rounded-lg overflow-hidden shadow relative">
            ...
            <a rel=prefetch href="{slug}" title="{title}" class="m-0 p-0">
              <img class="h-56 w-full object-cover object-center" src="{image?image:defaults.defaultImage}?w=1000&format=auto" alt="">
            </a>

          
          <div class="p-4 h-auto md:h-40 lg:h-48">
            <a href="{slug}" title="{title}" class="block text-orange-500 hover:text-gray-800 font-semibold mb-2 text-lg md:text-base lg:text-lg">
              {title}
            </a>
            ...
            {#if keywords}
                ...
                {#each keywords as keyword}
                    <a class="inline bg-gray-300 py-1 px-2 rounded-full text-xs lowercase text-gray-700" title="{keyword}" href="blog/{keyword}">#{keyword}</a>
                {/each}
                </div>
            {/if}
          </div>
